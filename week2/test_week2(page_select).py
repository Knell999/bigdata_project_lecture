...
def fetch_apartment_sales(lawd_cd, deal_ym, page_no=1, num_of_rows=10):
    serviceKey = 'PT7s57yQ6NYjF%2BK4IX03RFJISja7dPlVceXQn7%2Fm2DWnibRv2u9288PJ6m9t4hi6sH2A%2B8fa8AGF3nDiGmeZRw%3D%3D'
    url = f"http://openapi.molit.go.kr/OpenAPI_ToolInstallPackage/service/rest/RTMSOBJSvc/getRTMSDataSvcAptTradeDev?serviceKey={serviceKey}&LAWD_CD={lawd_cd}&DEAL_YMD={deal_ym}&pageNo={page_no}&numOfRows={num_of_rows}"
...
def main():
    ...
    page_no = st.sidebar.number_input("페이지 번호 입력", min_value=1, value=1)
    num_of_rows = st.sidebar.number_input("페이지당 행 수 입력", min_value=1, max_value=100, value=10)

    if st.sidebar.button("검색"):
        lawd_cd = region_dict[region]
        deal_ym = f"{year}{month:02d}"
        result_df = fetch_apartment_sales(lawd_cd, deal_ym, page_no, num_of_rows)
